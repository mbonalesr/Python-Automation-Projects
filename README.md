# 🐍 100 Days of Code: Python Automation Projects

This repository documents my journey through Udemy's course **"100 Days of Code: The Complete Python Pro Bootcamp"** by Dr. Angela Yu. 
Here, I apply logic, control flow, and data structures to build automation scripts and solve algorithmic challenges.

## 🛠️ Tech Stack
* **Language:** Python 3.x 🐍
* **Libraries:** Pandas 🐼, Tkinter 🖼️, Turtle 🐢
* **Tools:** VS Code, Git/GitHub
* **Focus:** OOP, Automation, GUI Development, Data Analysis

## 🚀 Progress Log

### Beginner Section: Scripting & Logic
* [X] **Day 1:** Band Name Generator *(Variables & Input)*
* [X] **Day 2:** Tip Calculator *(Data Types & Math Operations)*
* [X] **Day 3:** Treasure Island *(Control Flow & Logical Operators)*
* [X] **Day 4:** Rock Paper Scissors *(Randomization & Lists)*
* [X] **Day 5:** Password Generator *(Loops & Range)*
* [X] **Day 7:** Hangman Project
* [X] **Day 8:** Caesar Cipher
* [X] **Day 9:** Secret Auction *(Dictionaries & Nesting)*
* [X] **Day 10:** Calculator *(Functions with Outputs)*
* [X] **Day 11:** Blackjack Capstone Project
* [X] **Day 12:** Number Guessing Game
* [X] **Day 14:** Higher Lower Game

---

### Intermediate Section: OOP, GUIs & Data Handling
* [X] **Day 15-16:** Coffee Machine (Functional vs OOP)
* [X] **Day 17:** Quiz Project
* [X] **Day 18:** Hirst Painting
* [X] **Day 19:** Turtle Race
* [X] **Day 20-21:** Classic Snake Game *(Inheritance & Slicing)* 🐍
* [X] **Day 22:** Classic Pong Game *(State Management)* 🏓
* [X] **Day 23:** Turtle Crossing Capstone
* [X] **Day 24:** Mail Merge Project *(File System)* ✉️
* [X] **Day 25:** US States Game *(CSV & Pandas)* 🗺️
* [X] **Day 26:** NATO Alphabet Project *(Comprehensions & Pandas)* 🔡 
* [X] **Day 27:** GUI Mi to Km Converter *(Tkinter)* ⚖️
* [X] **Day 28:** Pomodoro Timer *(Tkinter, Dynamic Typing & UI)* ⏳
* [X] **Day 29-30:** Password Manager *(Tkinter, JSON & Exception Handling)* 🔐

---

### Intermediate+ Section: APIs, Email & Automation
* [X] **Day 32:** Kanye Quotes *(API Endpoints)* 🗣️
* [X] **Day 33:** Automated Birthday Wisher *(smtplib & datetime)* 🎂
* [X] **Day 31:** Flashy CAE Flashcards *(Capstone Project: Tkinter & Pandas)* 🇬🇧
* [X] **Day 35:** Quizzler App *(API & Tkinter UI)* 🧠
* [X] **Day 34:** Automatic Bitcoin Monitor *(API Requests)* 💰 -  In progress 🚧
* [X] **Day 35:** Rain Alert Project *(API Authentication & Environment Variables)* ☔
* [ ] **Day 36:** Stock Trading Alert *(Multiple APIs & Automation)* 📈 -  In progress 🚧
* [ ] **Day 37:** Habit Tracker *(HTTP POST Requests & Headers)* 🏃‍♂️ -  In progress 🚧
* [ ] **Day 38:** Workout Tracker *(NLP, Nutritionix & Sheety APIs)* 💪 - Coming soon!
* [ ] **Day 39-40:** Flight Deal Finder *(OOP & Multi-API Orchestration)* ✈️ - Coming soon!
* [ ] **Day 45:** 100 Movies Project *(BeautifulSoup & HTML Parsing)* 🎬🍿- Coming soon!